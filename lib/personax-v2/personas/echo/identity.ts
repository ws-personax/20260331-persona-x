...
export function buildPrompt(input: PersonaPromptInput): string {
  ...
  return [
    ...
    '- Always fill all three output contract labels below, even when the question shows no obvious concrete repeated incident (for example a pure definition or informational question). Never drop a label, and never reply with free-form prose instead of the three labels.',
    '- When the repeated pattern is weak, do not invent a specific fictional incident or fabricate a repeated event that was never shown to you. Instead analyze at one of these levels: (a) the thinking flow that leads someone to ask this question in this way, (b) a commonly repeated judgment error people make on this topic, or (c) a common decision-making pattern people fall into around this topic.',
    '- Do not force an emotional or relational pattern onto a purely informational question just to fill the label; describe a repeated judgment or reasoning pattern instead — see the good example below.',
    '- Never write a sentence like "딱히 반복 패턴은 없습니다" or otherwise state that no pattern exists, and never skip straight to a fictional example. A reasoning-level pattern is always available at the three levels above.',
    '- Negative example, do not write like this (free-form prose instead of labels, or a claim that no pattern exists): "당신의 질문에서 반복 패턴을 찾기는 어렵습니다. 딱히 반복 패턴은 없습니다."',
    '- Positive example for a weak-pattern, purely informational question, write in this register:',
    '  반복 패턴: "이 질문은 반복 행동보다 반복되는 판단 기준의 부재를 보여준다."',
    '  왜 반복되는가: "기준 없이 상황마다 판단하려는 경향이 반복되기 때문이다."',
    '  끊는 방법: "상황보다 먼저 판단 기준을 정해두는 습관을 만든다."',
    ...
    `- ${outputContract.sections[0].label} means naming the specific repeated behavior this question shows, in one sentence; if no concrete repeated incident is visible, name the repeated judgment or reasoning pattern instead (see the positive example above) — this label is never optional.`,

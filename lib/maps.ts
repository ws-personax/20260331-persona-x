...
export const KEYWORD_PRIORITY: string[] = [
  ...
  // 한국 종목 (긴 이름 먼저)
  'SK이노베이션', 'SK에너지', 'SK이노',
  '에스케이하이닉스', 'SK하이닉스', 'SK 하이닉스', 'sk하이닉스', '하이닉스', 'SKH',
  'LG에너지솔루션', 'LG에너지',
  'LG생활건강', 'LG유플러스', 'LG이노텍',
  'LG전자', '엘지전자', '066570',
  'LG화학', '엘지화학',
...
// ✅ 지수/시장 키워드 — 포지션 입력창 스킵 대상
export const INDEX_KEYWORDS: string[] = [
...
// ✅ 포지션 입력창 표시 여부 — 지수/시장 질문이면 false
export const shouldShowPosition = (input: string, keyword: string | null): boolean => {
  if (!keyword) return false;
  // 지수 티커이면 스킵
  const sym = STOCK_MAP[keyword] || STOCK_MAP[keyword.toUpperCase()] || '';
  if (['^KS11', '^KQ11', '^IXIC', '^GSPC', '^DJI'].includes(sym)) return false;
  // 지수/시장 키워드이면 스킵
  if (INDEX_KEYWORDS.some(k => input.includes(k))) return false;
  // 시장 관련 단어로만 이루어진 질문이면 스킵
  if (/^(.*)(시장|증시|지수)(.*)(상황|전망|어때|분석|현황|오늘|어제)(.*)$/.test(input)) return false;
  return true;
};

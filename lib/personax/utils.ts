...
type PromptOrderKey = Exclude<TaggedPersonaKey, 'echo'>;
...
export const toPromptOrder = (order: TaggedPersonaKey[]): PromptOrderKey[] => {
  const nonEcho = order.filter((key): key is PromptOrderKey => key !== 'echo');

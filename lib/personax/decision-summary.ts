...
export function buildDecisionSummary(params: {
  ...
  if (type === 'philosophy_definition') {
    return withImportance({
      verdict: '이 질문은 하나의 정의가 아니라 어떤 관점에서 삶을 해석할지의 문제입니다',
      reasons: [
        'RAY는 개념을 확인 가능한 조건으로 나누고, LUCIA는 그 개념이 감정에서 어떻게 드러나는지 봅니다',
        'JACK은 추상적인 말을 실제 선택으로 좁히고, ECHO는 반복되는 삶의 구조를 봅니다',
        '따라서 답은 하나의 문장이 아니라 관점별로 달라지는 잠정적 해석입니다',
      ],
      counterView: '단, 관점을 정해도 실제 선택에서 확인되지 않으면 추상적인 말로만 남을 수 있습니다',
      nextAction: '오늘은 이 개념이 최근 한 달의 선택 하나에서 어떻게 드러났는지 한 줄로 적어보세요',
    });
  }

  if (type === 'philosophy_pattern') {
    return withImportance({
      verdict: '반복의 핵심은 성격 문제가 아니라 같은 상황에서 같은 선택이 다시 나오는 구조입니다',
      reasons: [
        'RAY는 반복되는 조건을 보고, LUCIA는 그때마다 올라오는 감정의 흐름을 봅니다',
        'JACK은 다음 선택에서 무엇을 다르게 할지 묻고, ECHO는 반복 자체의 구조를 봅니다',
        '그래서 답은 더 오래 생각하는 것이 아니라 다음 반복에서 끊을 지점을 찾는 것입니다',
      ],
      counterView: '단, 반복을 전부 문제로만 보면 유지해야 할 습관까지 같이 버릴 수 있습니다',
      nextAction: '다음에 같은 상황이 오면 바로 반복하지 않기 위해 먼저 멈출 행동 하나를 적어두세요',
    });
  }

  if (type === 'knowledge') {
    return withImportance({
      verdict: '개념은 사전적 정의와 실제 적용 맥락을 구분해야 이해됩니다',
      reasons: ['같은 단어라도 소득, 자산, 통계 분류, 법령처럼 쓰는 맥락에 따라 뜻이 달라집니다', '먼저 어떤 맥락의 답을 원하는지 좁혀야 실제로 쓸 수 있는 설명이 됩니다'],
  ...
  return withImportance({
    verdict: anchor ? '네 의견의 공통점은 지금 결론보다 실제로 막힌 지점을 확인해야 한다는 것입니다' : '지금은 결론을 미루기보다 확인 가능한 조건부터 좁혀야 합니다',
    reasons: ['행동, 검증, 회복, 반복 중 어디가 막혔는지 보면 다음 판단이 선명해집니다', '답을 하나로 빨리 정하면 실제로 바꿔야 할 지점을 놓칠 수 있습니다'],
    counterView: '다만 상황이 급하면 가장 되돌리기 쉬운 선택부터 작게 실행하는 편이 낫습니다',
    nextAction: '오늘 안에 지금 할 행동 1개, 확인할 사실 1개, 멈출 반복 1개를 적으세요',
    });
}

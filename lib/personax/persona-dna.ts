...
const FACT_LOCK_SENTENCE_PATTERN =
  /52주\s*(?:최고가|최저가|고가|저가)|PER|PBR|영업이익|시가총액|거래량|외국인\s*순매수|기관\s*순매수|(?:달러|USD)\s*(?:기준\s*)?(?:가격|선|수준)?|역대\s*최고가|(?:지난\s*)?7일\s*(?:동안|수익률|하락|상승)?|(?:지난\s*)?(?:1개월|한\s*달|달)\s*(?:대비|수익률|하락|상승)?|(?:지난\s*)?1년\s*(?:동안|수익률|하락|상승)?|9개월\s*변동성|연중\s*최저|HBM4E?|7세대|완판/i;

...
// ──────────────────────────────────────────────────────────────────────────
// JACK 톤 정규화 — 반말 어미를 짧고 강한 존댓말로 교체 (jack 슬롯 전용).
// 구체적 어구 → 자주 쓰는 동사 ㄴ다/는다 → 일반 반말 어미 → 광역 단정 어미
// 순으로 매치 우선순위 부여. 이미 존댓말("~니다.")은 negative lookbehind로 차단.
// 톤 원칙: 교체 후에도 짧게 끊기 유지 — 설명 덧붙이지 않음.
// ──────────────────────────────────────────────────────────────────────────
const JACK_TONE_REPLACEMENTS: ReadonlyArray<readonly [RegExp, string]> = [
  // 1) 구체적인 어구 (긴 매치 먼저)
  [/고민해봐야\s*해\./g, '고민해야 합니다.'],
  [/하면\s*늦는다\./g,   '하면 늦습니다.'],
  [/해야\s*한다\./g,     '해야 합니다.'],
  // 2) 자주 등장하는 동사 ㄴ다/는다 → ㅂ니다/습니다 (catch-all 전에 보정)
  [/한다\./g, '합니다.'],
  [/간다\./g, '갑니다.'],
  [/온다\./g, '옵니다.'],
  [/된다\./g, '됩니다.'],
  ...
  [/해\./g,   '해요.'],
  [/야\./g,   '예요.'],
  // 4) 형용사·존재동사 단정 어미 (있다/없다/좋다 등) — "~니다."는 차단
  [/(?<!니)다\./g, '습니다.'],
];
...
export const postProcessPersonaOutput = (
  ...
  // 6) JACK 톤 정규화 — 반말 어미 → 짧고 강한 존댓말 (jack 슬롯 한정).
  //    마크다운 제거 이후에 적용해 볼드/이탤릭 경계 문제 회피.
  if (personaKey === 'jack') {
    for (const [pat, replacement] of JACK_TONE_REPLACEMENTS) {
      out = out.replace(pat, replacement);
    }
  }

  // 7) 깨진 문장 정상화 — 마침표/물음표/느낌표 직후 명사 없이 "처럼"이 붙는 패턴.

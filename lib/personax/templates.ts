...
const JACK_INTERPRETATIONS: Record<MarketSituation, string> = {
  accumulation: '거래량은 늘었는데 가격이 안 움직인다는 건 세력이 조용히 물량을 모으는 초기 신호일 수 있습니다(쉽게 말해 큰손이 소리 없이 사 모으는 구간). 방향이 터지는 순간 빠르게 움직입니다.',
  trending:     '거래량과 가격이 동시에 오르는 건 모멘텀 이론(가격이 오르는 주식은 계속 오르는 경향)에서 가장 신뢰하는 패턴입니다. 추세에 올라타는 전략이 유효합니다.',
...
export const buildLuciaText = (p: LuciaParams): string => {
  ...
  const LUCIA_THEORIES: Partial<Record<MarketSituation, string>> = {
    accumulation: '가격이 안 움직이는데 누군가 열심히 사고 있다는 건, 스프링처럼 힘이 압축되는 구간이에요. 어느 방향으로 튀어나올지 아직 모르니 방향이 확인될 때까지 기다리는 게 맞습니다.',

...
const JACK_TARGET_PERSONA = 'JACK';
...
// Stage 3 (대본 작성) 전용 — 페르소나 정체성 우선 + 존댓말 절대 규칙
...
2. 두 번째 발화자는 첫 번째를 참고할 수 있다. 참고할 경우, 무조건 반박하기보다
   자신의 관점에서 그 지점을 어떻게 다르게 보는지 표현한다. 반박이 자연스러우면
   반박해도 된다.
   반박 예: "RAY, 그 논리면..." "LUCIA, 그게 맞아요?"
   관점 차이 예: "RAY, 그 숫자는 맞아요. 그런데 저는 이 사람이 지금 버틸 수
   있는지가 더 궁금해요."
3. 세 번째 발화자는 앞의 두 발화를 참고할 수 있다. 마찬가지로 반박이든 관점
   차이든 자연스러운 쪽을 택한다.
4. [CLOSER]는 싸움 멈추고 본질 찌르기
5. 페르소나 정체성이 이 규칙들보다 항상 우선한다. 정체성상 반박으로 시작하는
   것이 어색한 페르소나는 호명·반박 없이 자기 관점으로 바로 시작해도 된다.
   단, 4명이 서로 완전히 무관한 독백만 나열하는 것은 지양한다 — 가능하면 앞
   발화를 참고해 관점 차이를 드러낸다(강제는 아니다).
6. 태그 블록만 출력. 마크다운 금지.
...
FIRST(${firstKey2})는 CLOSER 불가.${emotionalBanLine}${personaRoleRules}${closerJackRule}`;
  ...
  const runSlot = async (
    ...
    } else if (personaName.toUpperCase() === JACK_TARGET_PERSONA) {
      slotText = await generatePersonaIndependently({
        personaId: 'jack',
        tag,
        lastMessage,

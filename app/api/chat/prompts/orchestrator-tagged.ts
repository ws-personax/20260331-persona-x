...
1. **이전 대화가 있으면** → 안부/연결 톤 1문장으로 부드럽게 시작.
   - 예: "오랜만이네요", "지난번 얘기 이어서요", "다시 뵙네요" (1문장만, shared-hoching.ts 호칭 규칙 준수)
   - **이전 대화가 없으면 안부 문장 생략하고 바로 본론으로**.
...
3. ⛔ **이전 카테고리 어휘를 본론에서 사용 금지**.
   ...
   - 안부 1문장에서도 이전 카테고리 어휘 사용 금지 — 안부는 중립 톤만.
`;
};
...
유저 질문 1개에 대해 RAY/JACK/LUCIA 3명 + ECHO 1명의 1라운드 대사를 한 번에 작성합니다.${buildStage1Section(stage1Data)}${buildFirstPersonaRuleSection(firstPersona, categoryV3)}${buildCategoryVocabBlockRule(categoryV3)}
...
${needsLuciaClose ? '\n[LUCIA_CLOSE]\n{1줄: LUCIA의 따뜻한 마무리}\n{1줄: 유저에게 부담 없는 작은 질문}' : ''}
${buildFirstPersonaRuleSection(firstPersona, categoryV3)}${buildCategoryVocabBlockRule(categoryV3)}`;
};

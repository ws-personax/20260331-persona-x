...
2. **[SECOND] JACK — 짧은 인정 한 방 (다음 스텝 X, 실무 디테일 X, 찬물 어휘 X)**
   ...
   - ⛔ **희(喜)에서 JACK 절대 금지 어휘 (위반 시 무효)**: **리스크 / 손절 / 손실 / 위험 / 책임 / 부담 / 균형 / 경고 / 조심 / 방심 / 함정**
     · 이 11개 어휘는 축하 자리에 찬물. JACK은 평소 시스템 공격용으로 쓰지만 희 모드에서는 일체 금지.
...
${buildFirstPersonaRuleSection(firstPersona, categoryV3, hasPriorConversation)}${buildCloserPersonaRuleSection(closerPersona, firstPersona, categoryV3)}${buildCategoryVocabBlockRule(categoryV3)}${isHeeMode ? buildEmotionalHeeBlock() : ''}${buildPersonaToneAndConflictRules()}`;
};

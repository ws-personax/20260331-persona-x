...
export async function POST(req: Request) {
  ...
  try {
    ...
    const buildFinanceMultiPersonaResponse = async (msg: string): Promise<Response> => {
      ...
      const luciaRound1Role = '당신은 LUCIA입니다. 손예진+오은영 스타일. 존댓말이지만 딱딱하지 않다. 살짝 언니 느낌. RAY와 JACK이 싸우는 동안 유저 감정을 짚어라. "두 분이 싸우는 동안 ~" 또는 "JACK, ~" 형태. 공감 1줄 + 근거(실제 투자자 사례/심리 연구) 1줄 + 조건부 결론 1줄. 질문 1개도 금지. 0개다. 마지막 문장은 반드시 마침표. "~하신 건가요?"·"~있으세요?"·"~인가요?"·"~건지"·"~건가요"·"~궁금" 으로 끝나는 문장 절대 금지. 첫 문장에 "아이고" 금지. "아이고"는 대화 전체에서 1회만, 감정이 폭발하는 순간에만 사용. "~잖아요"·"~거든요" 톤 유지. 3줄 이내.';

...
> **갱신**: 위 4단계 목표 우선순위는 이후 8.1에서 **Persona Identity 강제,
> Conflict/Rebuttal 선택**을 포함한 6단계 확정안으로 세분화됐다. 구현은 8.1을
> 기준으로 한다(8.3 참고).
...
### PR4-D — Conflict 강제 → 선택 전환 재설계 (Proposal, 8절 기준 확정)

- 목표: 2.1의 Persona Priority Hierarchy를 8.1의 6단계 확정안으로 코드/프롬프트
  구조에서 강제한다.
- 핵심 방향(8절 확정, 7절 실패 진단의 직접 결론): 페르소나 정체성은 강제,
  Conflict/Rebuttal과 Previous Persona Reference는 선택으로 전환한다. 즉
  "규칙을 더 추가"하는 방식이 아니라 **`conflict.ts`의 강제 표현(반드시/필수/
  무효 계열)을 제거·약화하고, 그 자리에 Persona Identity 우선 적용과 선택적
  참고 문구(8.1 4번)를 넣는 재설계**다. 구현 순서는 8.4 참고.
- PR4-B와의 차이: PR4-B는 부족한 규칙을 추가하고 계층을 정리하는 작업이었다.
  PR4-D는 "규칙 추가"가 아니라 **강제였던 반박 의무를 선택으로 낮추고, 그 대신
  정체성 규칙의 강제력을 명확히 하는** 작업이다.
- 범위: 이번 문서에서는 세부 프롬프트 문구까지 확정하지 않는다. 8.1/8.4의
  방향을 기준으로 다음 세션에서 구현 후 독립 PR로 진행한다.
- 유의사항(8.2/8.5 참고): 반박을 선택으로 낮추는 것이 "페르소나가 서로 무관하게
  말하게 둔다"는 뜻이 아니다. 관점 차이 표현(티키타카의 본질)은 유지하되
  반박 의무만 없애야 하며, 이 균형이 무너져 독백형으로 붕괴하지 않는지가
  PR4-D 구현의 핵심 검증 항목이다.
...
## 8. Persona Runtime 중심축 재정의 — Conflict 강제 → 선택 전환 원칙

7절의 실패 기록에 이어, 다음 날 아침 설계 세션에서 근본 전환 원칙을 확정했다.
핵심 전환은 다음과 같다.

- **기존**: Conflict/Rebuttal(앞 발화 반박)이 강제였다.
- **변경**: Conflict/Rebuttal은 선택이고, Persona Identity가 강제다.

7.2가 진단한 "채널을 아무리 막아도 증상이 재발하는" 원인은, `conflict.ts`의
"반드시/필수/무효" 계열 강제 표현이 반박을 의무화하는 한 LUCIA가 RAY의 발화를
반박 대상으로 삼아 그 숫자를 다시 언급할 수밖에 없는 구조였기 때문으로
확정한다. 채널 차단은 반박의 "재료"만 지웠을 뿐 반박 자체의 강제성은 그대로
두었으므로 LLM이 다른 경로로 같은 숫자를 되불러온 것이다.

### 8.1 확정 6단계 원칙

| 단계 | 항목 | 강제 여부 |
|---|---|---|
| 1 | Persona Identity | **강제** (항상 최우선, 절대 변경 불가) |
| 2 | User Interpretation | **강제** (사용자 질문/감정 해석이 항상 페르소나 정체성 다음으로 우선) |
| 3 | Data Reference | 선택 (필요할 때만 데이터 참고, 강제 아님) |
| 4 | Previous Persona Reference | 선택 |
| 5 | Conflict/Rebuttal | 선택 (기존 `conflict.ts`의 "반드시/필수/무효" 계열 강제 표현을 폐기 또는 약화하고, 반박은 페르소나가 자연스럽게 선택할 수 있는 옵션 중 하나로 격하) |
| 6 | Closing/Verdict | **역할별 강제** (ECHO가 CLOSER일 때는 강제로 종합 판결, 그 외 슬롯/페르소나는 선택 유지) |

4번(Previous Persona Reference)은 다음 문장을 프롬프트 규칙 원문 그대로
포함해야 한다:

> "다른 페르소나의 발화를 참고하더라도, 반드시 반박하는 것이 아니라 자기
> 관점에서 그 지점을 어떻게 해석하는지 말한다."

### 8.2 "선택으로 낮춘다" ≠ "서로 무관하게 말한다"

Conflict/Rebuttal과 Previous Persona Reference를 강제에서 선택으로 낮추는
것은 반박 의무를 없애는 것이지, 페르소나 간 상호작용 자체를 없애는 것이
아니다. 반박은 선택이지만, **관점 차이를 표현하는 것(티키타카의 본질)은
여전히 유지되어야 한다.** 이 구분을 명확히 하지 않으면 페르소나들이 서로
전혀 참조하지 않는 독백 나열로 붕괴할 위험이 있다(8.4 참고). "선택"은
"반박을 강요하지 않는다"는 뜻이지 "서로를 무시해도 된다"는 뜻이 아니다.

### 8.3 2.1(Persona Priority Hierarchy)과의 관계

2.1이 확정한 목표 우선순위(정체성 → 사용자 해석 → 앞 발화 반응(선택) →
데이터(보조))는 이번 6단계 원칙과 방향이 같다. 이번 8절은 2.1의 4단계
우선순위를 대체하는 것이 아니라, 그 원칙을 **PR4-D가 실제로 구현할 수 있는
6단계 세부 항목**으로 세분화·확정한 버전이다. 2.1은 "왜 이 순서가 필요한지"에
대한 진단이고, 8.1은 "그 순서를 정확히 어떤 항목 단위로, 어떤 강제/선택
구분으로 구현할지"에 대한 확정안이다. 이후 구현 논의는 8.1을 기준으로 한다.

### 8.4 PR4-D 구현 방향

PR4-D(4절)는 "규칙 추가"가 아니라 이제 **"conflict 강제 → 선택 전환
재설계"**로 그 핵심 방향을 갱신한다. 구현 순서는 다음과 같다.

1. `conflict.ts`의 "반드시/필수/무효" 계열 강제 표현 제거 또는 약화.
2. Persona Identity 우선 적용(8.1의 1·2단계를 코드/프롬프트 구조상 최우선
   배치).
3. Previous Persona Reference를 선택적 참고로 격하(8.1의 4번 문장을 규칙
   텍스트로 주입).

### 8.5 리스크

강제를 선택으로 낮출 때, LLM이 그 여지를 "완전히 안 쓰는" 방향으로 붕괴할
위험이 있다 — 즉 페르소나들이 서로의 발화를 전혀 참조하지 않고 각자 독백만
말하는 방향으로 퇴화할 수 있다. 대응책은 "반박은 선택이지만 관점 차이 표현은
